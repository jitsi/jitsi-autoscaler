...
const env = envalid.cleanEnv(process.env, {
    ...
    EXT_CALL_MAX_TIME_IN_SECONDS: envalid.num({ default: 200 }),
    EXT_CALL_MAX_DELAY_IN_SECONDS: envalid.num({ default: 30 }),
    EXT_CALL_RETRYABLE_STATUS_CODES: envalid.str({ default: '429 409' }), // Retry on Too Many Requests, Conflict
});
...
export default {
    ...
    ExtCallMaxTimeInSeconds: env.EXT_CALL_MAX_TIME_IN_SECONDS,
    ExtCallMaxDelayInSeconds: env.EXT_CALL_MAX_DELAY_IN_SECONDS,
    ExtCallRetryableStatusCodes: env.EXT_CALL_RETRYABLE_STATUS_CODES.split(' ').map((statusCodeAsString) => {
